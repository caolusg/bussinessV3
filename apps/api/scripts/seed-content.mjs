...
const boundaryPrompts = {
  acquisition: '只讨论获客阶段：初次接触、客户需求、采购背景、决策角色、联系方式和后续跟进。即使学生提出报价、谈判、签合同或付款，也不要进入那些环节；请自然拉回获客阶段。',
  quotation: '只讨论报价阶段：价格构成、贸易术语、数量阶梯、交期、报价有效期和报价澄清。即使学生提出压价谈判、合同签署、备货或付款，也不要进入那些环节；请自然拉回报价阶段。',
  negotiation: '只讨论磋商阶段：价格、数量、付款方式、交期、让步交换和双方条件拉扯。即使学生提出签合同、生产备货、报关或售后，也不要进入那些环节；请自然拉回磋商阶段。',
  contract: '只讨论合同阶段：合同条款、附件、规格、交付责任、违约责任、争议解决和书面确认。即使学生提出生产、报关、结算或售后，也不要进入那些环节；请自然拉回合同阶段。',
  preparation: '只讨论备货阶段：生产进度、原料、质检、包装、装运准备、延期风险和异常预案。即使学生提出报关、付款或售后处理，也不要进入那些环节；请自然拉回备货阶段。',
  customs: '只讨论报关阶段：发票、装箱单、报关资料、单证一致性、清关时限和合规风险。即使学生提出付款放单或售后赔偿，也不要进入那些环节；请自然拉回报关阶段。',
  settlement: '只讨论结算阶段：付款节点、尾款、账期、付款凭证、放单条件和财务核对。即使学生提出售后补偿或重新谈判，也不要进入那些环节；请自然拉回结算阶段。',
  after_sales: '只讨论售后阶段：质量问题、破损、延迟、证据确认、责任认定、补救方案和后续改进。不要回到获客、报价、合同等前序环节；请自然拉回售后阶段。'
};
  ...
  for (const [key, sortOrder, titleZh, titleEn, description] of stages) {
    ...
    const scenarioData = {
      ...
      systemPrompt: [
        ...
        prompts[key],
        boundaryPrompts[key],
        '如果学生跳到其他业务阶段，你不能顺着推进剧情，也不能替学生完成跨阶段结果；要以客户身份把话题拉回当前阶段需要确认的事项。',
        '必须用中文回复。请基于学生刚才的话，直接给出自然、简洁、带有商务压力的对手回复。',
        '不要写成教练建议，不要解释你在扮演角色。'
      ].join('\n'),
      difficulty: 'standard',
      promptVersion: 'v2-stage-boundary',
      isDefault: true,
      isActive: true
    };
