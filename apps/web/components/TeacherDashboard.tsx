...
const TeacherDashboard: React.FC<TeacherDashboardProps> = ({ user, onLogout, onPasswordChange }) => {
  ...
  const [downloadingDataExportAuditId, setDownloadingDataExportAuditId] = useState('');
  ...
  const downloadApprovedDataExportAudit = async (audit: DataExportAuditRow) => {
    if (audit.status !== 'approved' || audit.downloadedAt) return;
    setDownloadingDataExportAuditId(audit.id);
    setDataExportNotice({ tone: 'info', message: '正在准备已批准的数据下载...' });
    try {
      ...
      } else if (audit.exportType === 'research_ai_result' && researchAiResult?.rows?.length) {
        csv = toCsv(researchAiResult.rows);
      }

      if (!csv) {
        throw new Error('当前申请缺少可下载的数据，请重新提交下载申请。');
      }
      await finalizeApprovedCsvDownload(audit, csv, audit.fileName || `data-export-${Date.now()}.csv`);
    } catch (error) {
      const message = error instanceof Error ? error.message : '已批准数据下载失败';
      setDataExportNotice({ tone: 'error', message });
    } finally {
      setDownloadingDataExportAuditId('');
    }
  };
        ...
        <div className="overflow-x-auto rounded-xl border border-slate-100 bg-white">
          <table className="w-full min-w-[860px] text-left text-xs">
            ...
            <tbody className="divide-y divide-slate-100">
                ...
                return (
                  <tr key={row.id} className="align-top">
                    ...
                    <td className="px-3 py-2 text-right">
                      <button
                        type="button"
                        disabled={row.status !== 'approved' || Boolean(row.downloadedAt) || downloadingDataExportAuditId === row.id}
                        onClick={() => void downloadApprovedDataExportAudit(row)}
                        className={`inline-flex items-center justify-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-black transition ${
                          row.status === 'approved' && !row.downloadedAt
                            ? 'bg-indigo-600 text-white hover:bg-indigo-700 disabled:opacity-60'
                            : 'border border-slate-200 bg-slate-50 text-slate-400'
                        }`}
                      >
                        {downloadingDataExportAuditId === row.id ? <Loader2 size={13} className="animate-spin" /> : <Download size={13} />}
                        下载
                      </button>
                    </td>
                  </tr>
              ...
              {!rows.length && (
                <tr>
                  <td className="px-3 py-5 text-center text-sm font-bold text-slate-400" colSpan={6}>暂无下载申请</td>
                </tr>
              )}
            </tbody>
          </table>
